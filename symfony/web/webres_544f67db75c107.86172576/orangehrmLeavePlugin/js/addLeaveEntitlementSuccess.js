    ...
    function fetchEmployees(offset) {
        ...
        params = params + '&offset=' + offset + '&lt=' + $('#entitlements_leave_type').val() + '&fd='+$('#date_from').val()+ '&td='+ $('#date_to').val()+'&ent='+$('#entitlements_entitlement').val()+'&carryForward='+carryforwad;
        $.ajax({
            type: 'GET',
            url: getEmployeeUrl,
            data: params,
    ...
    $(document).ready(function() {               
        ...
        $('#btnSave').click(function() {
            if ($('#entitlements_carryForward').is(':checked')) {
                carryforwad=1;
            }else{
                 carryforwad=0;
            }
                ...
                if(!($('#entitlements_id').val() > 0)){
                    var valid = $('#frmLeaveEntitlementAdd').valid();
                        if (valid) {   
                            ...
                            params = 'empId='+$('#entitlements_employee_empId').val()+'&lt=' + $('#entitlements_leave_type').val() + '&fd='+$('#date_from').val()+ '&td='+ $('#date_to').val()+'&ent='+$('#entitlements_entitlement').val()+'&carryForward='+carryforwad;
        ...
        $('#dialogUpdateEntitlementConfirmBtn').click(function() {
            ...
            showProgressDialog(loadingMsg);   
            if(carryforwad == 1){
            var current=$('#entitlements_entitlement').val();
            $('#entitlements_entitlement').val(parseFloat(balance)+parseFloat(current))
            }
              //consol.log('m here'); consol.log($('#frmLeaveEntitlementAdd').val());consol.log('m @ end');
            $('#frmLeaveEntitlementAdd').submit();
        });
    ...
    $.validator.addMethod("valid_multiple", function(value, element, params) {
        ...
        $('#frmLeaveEntitlementAdd').validate({
                ...
                rules: {
                    ...
                    'entitlements[entitlement]': {
                        ...
                        twoDecimals: true,
                        min:1,
                        valid_multiple: $("#entitlements_entitlement").val(),
