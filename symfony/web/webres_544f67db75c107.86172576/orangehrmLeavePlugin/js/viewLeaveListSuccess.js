...
//       // if($('#'+this.id).val()==92 || $('#'+this.id).val()==94 || $('#'+this.id).val()==107 ){
//            
//            if($('#'+this.id).val()){
//            
//           // $('div.message').remove();
...
//            // Extracting the request id 
//            
//           // var id = $('#'+this.id).parent().siblings('input[id^="hdnLeaveRequest_"]').val();
//            var id = $('#'+this.id).parent().prev().find('input[id^="hdnLeaveRequest_"]').val();                      
//            $('#leaveId').val(id);
//            $('#leaveComment').val('');
//
//            fetchComments(id);
...
$("#conditionSave").click(function() {
        $('#conditionError').html('').removeClass('validation-error');
         var reviewDate = $('#leave_txtReviewDate').val().trim();
         if(reviewDate.length == 0){
             $('#dateError').html(lang_Required).addClass('validation-error');
             return;
         }
        var rawComment = $('#conditions').val().trim();
        if(rawComment.length > 250) {
            $('#conditionError').html(lang_length_exceeded_error).addClass('validation-error');
            return;
        } else if (rawComment.length == 0) {
            $('#conditionError').html(lang_Required).addClass('validation-error');
            return;                                
        }
        ...
        var reviewDate = $('#leave_txtReviewDate').val().trim();
        /* Setting the comment in the label */
       // alert($('#leaveId').val());return false;
        ...
        var data = {
            ...
            leaveComment: rawComment,
            reviewvalDate:reviewDate,
            token : $('#leaveComment__csrf_token').val()
        }
//alert(JSON.stringify(data, null, 4));return false;
        $.ajax({
            type: 'POST',
            url: conditionUpdateUrl,
            data: data,
            success: function(data) {
