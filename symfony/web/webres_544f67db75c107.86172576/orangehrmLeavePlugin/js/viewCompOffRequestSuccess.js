/*function handleSaveButton() {
    ...
    if (selectedActions > 0) {
                //alert(compOffRequestId);return false;
...
$(document).ready(function(){
    ...
    $('#view_request_comments').click(function() {
        ...
        $('#leaveId').val(compOffRequestId);
        $('#leaveOrRequest').val('request');        
        fetchComments(compOffRequestId);
    ...
    //open comments on  select of action
    /*
     ...
     $('.select_action').change(function(){                    
       if($('#'+this.id).val()){
        ...
        var id = $('#'+this.id).parent().prev().find('input[id^="hdnLeave_"]').val(); 
    ...
    $('select.select_action').bind("change",function() {
        ...
        if (mode == 'detailed') {
            $('#compoff-'+requestId).val($(this).val());
        } else {
...
//            if(!id){
//               id = $('#'+this.id).parent().prev().children(':first-child').next().val();
//            }
//            $('#leaveOrRequest').val('leave');
//            $('#leaveId').val(id);
//            $('#leaveComment').val('');
