...
class EmailService extends BaseService {
    ...
    private function _getMessage() {
        ...
        if (empty($this->messageFrom)) {
            ...
            $this->messageFrom = array($this->emailConfig->getSentAs() => 'Perennial HRM');
        }
    ...
    public function sendEmailNotificationByAction($to,$subject,$body){
        
        $this->emailConfig->getMailType();
        $this->_validateEmailAddress($to);
        $this->messageSubject = $subject;
        $this->messageTo = array($to);
        $this->messageBody = $body;
        return $this->sendEmail();   
    }
            ...
            if (!empty($template)) {                
                ...
                foreach ($allRecipients as $recipient) {
                    ...
                    if (!empty($to)) {
                        ...
                        try {
                            $message->setTo($to);
                            $message->setFrom(array($this->emailConfig->getSentAs() => 'Perennial HRM'));
