...
class EmployeeImmigrationDetailsForm extends sfForm {
	...
	$choise=array(
        ...
        $type_flag=array(
            ''=>__('Select'),
            EmployeeImmigrationRecord::TYPE_PASSPORT =>__('Passport'),
            EmployeeImmigrationRecord::TYPE_LICENSE =>__('Driving License'),
            EmployeeImmigrationRecord::TYPE_Adhar_Card =>__('Adhar Card'),
            EmployeeImmigrationRecord::TYPE_Election_Card =>__('Election Identity Card'),
            EmployeeImmigrationRecord::TYPE_PAN_Card =>__('PAN Card')
            
        );
        $recordType=array();
         foreach($this->empPassports as $record){
           $recordType[]=$record->type;
         }
       
        foreach($type_flag as $key=> $value){
            if(!in_array($key, $recordType) ){
              $choise[$key]  =$value;
            }
        }
...
//                    EmployeeImmigrationRecord::TYPE_PASSPORT => __('Passport'), EmployeeImmigrationRecord::TYPE_LICENSE => __('License')), 'default' => EmployeeImmigrationRecord::TYPE_PASSPORT));
        $widgets['type_flag']=new sfWidgetFormSelect(array('choices' =>$choise));
        ...
        $widgets['passport_expire_date'] = new ohrmWidgetDatePicker(array(), array('id' => 'immigration_passport_expire_date','minDate'=>$today));
...
//                        'choices' => array(EmployeeImmigrationRecord::TYPE_PASSPORT, EmployeeImmigrationRecord::TYPE_VISA))),
                'type_flag' => new sfValidatorString(array('required' => true)),
