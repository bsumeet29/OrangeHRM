...
class EmployeeReportToForm extends BaseForm {
            ...
            if ($existingReportToObject != null) {
                ...
                if ($this->getOption('reportToSupervisorPermission')->canCreate()) {
                    ...
                    $newReportToObject->setReportingMethodId($reportingType);
                     $supervisors = $this->getEmployeeService()->getSupervisorIdListBySubordinateId($selectedEmployee, true);
            foreach($supervisors as $supervisor){
                $this->assignIndirectSupervisor($supervisor,$empNumber,2);
            
            }
            $subordinates = $this->getEmployeeService()->getSubordinateIdListBySupervisorId($empNumber,true);
            foreach($subordinates as $subordinate){
                $this->assignIndirectSubordinate($selectedEmployee, $subordinate, 2);
            }
        ...
        if ($supOrSub == ReportTo::SUBORDINATE) {
            ...
            if ($existingReportToObject != null) {
                ...
                if ($this->getOption('reportToSubordinatePermission')->canCreate()) {
                    ...
                    $newReportToObject->setReportingMethodId($reportingType);
                      $subordinates = $this->getEmployeeService()->getSubordinateIdListBySupervisorId($selectedEmployee, true);
            foreach($subordinates as $subordinate){
                $this->assignIndirectSubordinate($empNumber,$subordinate,2);
            
            }
                  $supervisors = $this->getEmployeeService()->getSupervisorIdListBySubordinateId($empNumber, true);
            foreach($supervisors as $supervisor){
                $this->assignIndirectSupervisor($supervisor,$selectedEmployee,2);
    ...
    public function assignIndirectSupervisor($supervisor,$empNumber,$reportingType){
         $existingReportToObject = $this->getEmployeeService()->getReportToObject($supervisor, $empNumber);
if ($existingReportToObject == null) {
    $newReportToObject = new ReportTo();
                    $newReportToObject->setSupervisorId($supervisor);
                    $newReportToObject->setSubordinateId($empNumber);
                    $newReportToObject->setReportingMethodId($reportingType);
                    $newReportToObject->save();
}
    }
    public function assignIndirectSubordinate($supervisor,$subordinate,$reportingType){
         $existingReportToObject = $this->getEmployeeService()->getReportToObject($supervisor, $empNumber);
if ($existingReportToObject == null) {
    $newReportToObject = new ReportTo();
                    $newReportToObject->setSupervisorId($supervisor);
                    $newReportToObject->setSubordinateId($subordinate);
                    $newReportToObject->setReportingMethodId($reportingType);
                    $newReportToObject->save();
}
    }
