...
class EmployeeMembershipForm extends BaseForm {
    ...
    private function getMembershipWidgets() {
        ...
        $memberships = array_merge(array(''),$memberships);
        
        $commutationType = array('' => "-- " . __('Select') . " --", 'Public' => __('Public'), 'Private' => __('Private'));
        $currency = $this->getCurrencyList();
        
        $widgets = array();
        /*$widgets['membership'] = new sfWidgetFormSelect(array('choices' => array('' => "-- " . __('Select') . " --", '2 wheeler' => __('2 wheeler'), '4 wheeler' => __('4 wheeler'))));
        ...
        $widgets['type'] = new sfWidgetFormSelect(array('choices' => $commutationType));
        $widgets['vehicleType'] = new sfWidgetFormSelect(array('choices' => array('' => "-- " . __('Select') . " --", '2 wheeler' => __('2 wheeler'), '4 wheeler' => __('4 wheeler'))));
        $widgets['vehicleNumber'] = new sfWidgetFormInputText();
        return $widgets;
    }
    ...
    private function getMembershipValidators() {
        ...
        $commutationTypes = array('Private'=>'Private','Public'=>'Public');
        $vehicleTypes = array('2 wheeler'=>'2 wheeler','4 wheeler'=>'4 wheeler');
        $validators = array(
            ...
            'subscriptionRenewalDate' => new ohrmDateValidator(array('date_format' => $inputDatePattern, 'required' => false),
                    array('invalid' => 'Date format should be ' . $inputDatePattern)),*/
            'type' => new sfValidatorChoice(array('required' => true,'choices' => array_keys($commutationTypes))),
            'vehicleType' => new sfValidatorChoice(array('required' => FALSE,'choices' => array_keys($vehicleTypes))),
           // 'vehicleNumber' => new sfValidatorString(array('required' => false,'trim'=>true)),
            'vehicleNumber' =>new sfValidatorRegex(array('pattern' => '/^[a-zA-Z0-9 \-]*$/','trim'=>true,'required' => FALSE)),
        );
        return $validators;
    }
    ...
    public function save() {
        ...
        if ($allowed) {
           ...
           /* $membershipDetail->subscriptionCommenceDate = $this->getValue('subscriptionCommenceDate');
            ...
            $membershipDetail->type = $this->getValue('type');
            $membershipDetail->vehicleType = $this->getValue('vehicleType');
            $membershipDetail->vehicleNumber = $this->getValue('vehicleNumber');
