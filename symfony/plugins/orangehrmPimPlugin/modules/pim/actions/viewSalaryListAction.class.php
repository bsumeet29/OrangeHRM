...
class viewSalaryListAction extends basePimAction {
    ...
    public function execute($request) {
        ...
        $oldEmpId=$employee->getEmployeeId();
        $oldEmpPaymentMethod=$employee->getPaymentMode(); 
        $workEmailId=$employee->getEmpWorkEmail(); 
            ...
            if ($this->form->isValid()) {
                ...
                if ($this->salaryPermissions->canCreate() || $this->salaryPermissions->canUpdate()) {
                    ...
                    if ($directDebitOk) {
                        ...
                        $newEmpId=$employee->getEmployeeId();
                        $newEmpPaymentmethod=$employee->getPaymentMode();
                        $this->getEmployeeService()->saveEmployee($employee);
                        if($newEmpId != $oldEmpId)
                        {
                            $subject="Employee Id Change.";
                            $mailMessage=" Dear ".$fname." ".$lname.",<br/><br/>";
                           
                            $mailMessage.="Your employee Id changed by Admin <br /><br />";
                            $mailMessage .= "<b>Employee Id: </b> ". $newEmpId ."<br/><br/>";
                            ...
                            $emailService->sendEmailNotificationByAction($workEmailId,$subject,$mailMessage);
                        }
