...
class SystemUserDao extends BaseDao {
    ...
    public function saveSystemUser(SystemUser $systemUser) {
        try {
            ...
            $employee=$systemUser->getEmployee();
            $employee->setEmpWorkEmail($systemUser->getUserName());
            $employee->save();
            $systemUser->save();
            return $systemUser;
        } catch (Exception $e) {
...
//            print($query->getSqlQuery());
    ...
    public function isExistingSystemUserByEmpId($empId, $userId = null){
        try {
            $query = Doctrine_Query:: create()->from('SystemUser u');
             if($empId !=0){
                    $query->andWhere('u.emp_number = ?', $empId);
             }
             if (!empty($userId)) {
                    $query->andWhere('u.id != ?', $userId);
             }
...
//                    ->set('u.deleted', 1)
    ...
    public function updatePassword($userId, $password) {
     ...
     public function getEmpNumbers($ids){
         $query = Doctrine_Query::create()
                   ->select('s.emp_number')
                   ->from('SystemUser s')
                   ->whereIn('s.id', $ids);
