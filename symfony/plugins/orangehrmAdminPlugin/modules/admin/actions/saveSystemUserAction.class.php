...
class saveSystemUserAction extends sfAction {
    ...
    public function execute($request) {
        ...
        if ($request->isMethod('post')) {
            ...
            if ($this->form->isValid()) {
                ...
                $this->_validateEmailAddress($username);
                $userId = $this->form->getValue('userId');
                $emplyeeData=  $this->form->getValue('employeeName');
                $emplyeeId=$emplyeeData['empId'];
                
                //check username for employee is exsist or not
                if(isset($emplyeeId) && $emplyeeId != "" ){
                     $user  =  $this->getSystemUserService()->isExistingSystemUserByEmpId($emplyeeId,$userId);
                     $isExisting      =   ( $user instanceof SystemUser)?true:false;
                     if($isExisting){
                     $this->getUser()->setFlash('warning', __('Failed To Save: Username for employee already exsist.'));
                     $this->redirect('admin/saveSystemUser');
                     }
                }
    ...
    private function _validateEmailAddress($emailAddress) {
        ...
        if (!preg_match("/^[^@]*@[^@]*\.[^@]*$/", $emailAddress)) {
            $this->getUser()->setFlash('warning', __('Failed To Save: Invalid email address'));
            $this->redirect('admin/saveSystemUser');
        }
