 ...
 */
class updateCompOffCommentAction extends BaseLeaveAction {
   ...
   /* public function getCompOffRequestService() {
    ...
    public function execute($request) {
        ...
        $compOffRequestId = trim($request->getParameter("leaveRequestId"));
        $compOffId = trim($request->getParameter("leaveId"));
        ...
        if ($compOffRequestId != "") {
            $compOffRequest = $this->getCompOffRequestService()->fetchCompOffRequest($compOffRequestId);     
            $permissions = $this->getCommentPermissions($loggedInEmpNumber == $compOffRequest->getEmpNumber());
            if ($permissions->canCreate()) {
                $form = new LeaveCommentForm( array(),array(),true);
                if ($form->getCSRFToken() ) {//== $request->getParameter("token")
                    $savedComment = $compOffRequestService->saveCompoffRequestComment($compOffRequestId, 
                        $comment, $createdBy, $loggedInUserId, $loggedInEmpNumber);
                    $this->getCompOffCommentUpdateDataForEmail($savedComment,$compOffRequest,'comp-off request');
                }
            }
        }
        if ($compOffId != "") {
            $compOff = $this->getCompOffRequestService()->readCompOff($compOffId);
            $permissions = $this->getCommentPermissions($loggedInEmpNumber == $compOff->getEmpNumber());
            if ($permissions->canCreate()) {
                $form = new LeaveCommentForm( array(),array(),true);
                if ($form->getCSRFToken()) {
                $savedComment = $compOffRequestService->saveCompoffComment($compOffId, 
                    $comment, $createdBy, $loggedInUserId, $loggedInEmpNumber);
                $this->getCompOffCommentUpdateDataForEmail($savedComment,$compOff,'comp-off');
                }
            }
        }
    ...
    private function getCompOffCommentUpdateDataForEmail($savedComment,$request,$type){
        $user = $this->getUser();
        $loggedInEmpNumber = $user->getAttribute('auth.empNumber');
