 ...
 * @author firoj
 */
class deleteCompOffHistoryAction extends baseCoreLeaveAction {
      ...
      public function execute($request) {
        ...
        if (count($ids) > 0) {
            try{
                if ($form->isValid()) {
                    $this->getCompOffRequestService()->deleteCompOffHistory($ids);
        ...
        $this->redirect('leave/viewCompOffList?reset=1');
    }
//    public function execute($request) {
//        print_r($request);
