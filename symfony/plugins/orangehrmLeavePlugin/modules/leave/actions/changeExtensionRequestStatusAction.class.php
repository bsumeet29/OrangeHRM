 ...
 * changeLeaveStatus
 */
class changeExtensionRequestStatusAction extends baseCoreLeaveAction {
    ...
    public function execute($request) {
        ...
        if ($request->isMethod('post')) {
          ...
          // $allowed = $manager->areEntitiesAccessible('Employee', $empNumbers);
           ...
           /* if ($allowed) {
            ...
            if ($form->isValid()) {
                $this->getCompOffRequestService()->changeCompoffExtensionStatus($changes); 
        ...
        if ($changedByUserType == SystemUser::USER_TYPE_EMPLOYEE) {
           ...
           if (trim($request->getParameter("id")) != "") {
            ...
            $redirectAction = stripos($request->getReferer(), 'viewMyCompOffList') === FALSE ? 'leave/viewCompoffExtensionRequests' : 'leave/viewMyCompOffList';
