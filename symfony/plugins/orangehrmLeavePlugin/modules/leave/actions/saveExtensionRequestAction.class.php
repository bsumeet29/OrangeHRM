 ...
 * updateComment
 */
class saveExtensionRequestAction extends baseLeaveAction {
    ...
    public function execute($request) {
        ...
        $entitlementId = trim($request->getParameter("entitlementId"));
        //$leaveId = trim($request->getParameter("leaveId"));
        $comment = trim($request->getParameter("extensionComment"));
        $reviewDate = trim($request->getParameter("extensionDate"));
        $user = $this->getUser();
       ...
       // echo"logged :".$loggedInEmpNumber;exit;
        ...
        $savedComment = NULL;
        if ($entitlementId != "") {
            //$leaveRequest = $this->getLeaveRequestService()->fetchLeaveRequest($entitlementId);  
            //$permissions = $this->getCommentPermissions($loggedInEmpNumber == $leaveRequest->getEmpNumber());

            //if ($permissions->canCreate()) {
                $form = new CompoffExtensionForm( array(),array(),true);
                //if ($form->getCSRFToken() == $request->getParameter("token")) {
                $reviewDate = strtotime($reviewDate);
                $reviewDate = date('Y-m-d',$reviewDate);
                    $savedComment = $compoffRequestService->saveCompoffExtensionRequest($entitlementId, $comment, $reviewDate);
