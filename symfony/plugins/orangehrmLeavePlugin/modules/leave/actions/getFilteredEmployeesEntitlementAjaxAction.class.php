...
class getFilteredEmployeesEntitlementAjaxAction  extends sfAction {
    ...
    protected function getMatchingEmployees($parameters) {
        ...
        foreach($employees as $employee) {
            ...
            if(isset($parameters['carryForward'])){
            $leaveEntitlementSearchParameterHolder->setCarryForward($parameters['carryForward']);
            }
...
//                 if($parameters['carryForward']==1){
//                 $usedday=$existingLeaveEntitlement->getDaysUsed() ;
...
//            } 
            foreach ($entitlementList as $existingEntitlement) {
                ...
                if($parameters['carryForward']==1 ){
                   // echo 'set carray in emp';
                $usedday=$existingEntitlement->getDaysUsed() ;
                $oldValue=$oldValue-$usedday;
                }
            }
