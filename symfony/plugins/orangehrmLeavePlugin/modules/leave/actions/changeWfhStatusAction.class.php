 ...
 * changeLeaveStatus
 */
class changeWfhStatusAction extends baseCoreLeaveAction {
    ...
    public function execute($request) {
        if ($request->isMethod('post')) {
            ...
            $empNumbers = $this->_getWfhChangeEmployeeNumbers($changeIds, $changeType);
            ...
            if ($form->isValid()) {
                $this->getWfhRequestService()->changeWfhStatus($changes, $changeType, $changeComments, $changedByUserType, $_SESSION['empNumber']);                
        ...
        if ($changedByUserType == SystemUser::USER_TYPE_EMPLOYEE) {
            ...
            if ($changeType == 'change_leave') {
                $url = "leave/viewWfhRequest";
                $this->getUser()->setFlash('myLeave', true);
            } else {
                $url = "leave/viewMyWfhList";
            }

            $url = ($changeType == 'change_leave') ? "leave/viewWfhRequest" : "leave/viewMyWfhList";
            ...
            $redirectAction = stripos($request->getReferer(), 'viewMyWfhList') === FALSE ? 'leave/viewWfhList' : 'leave/viewMyWfhList';
            
            $url = ($changeType == 'change_leave') ? "leave/viewWfhRequest" : $redirectAction;
    ...
    protected function _getWfhChangeEmployeeNumbers($changes, $changeType) {
        $empNumbers = array();
        ...
        if ($changeType == 'change_leave_request') {
            ...
            foreach ($changes as $id) {
                $wfhRequest = $wfhRequestService->fetchWfhRequest($id);
                $empNumbers[] = $wfhRequest->getEmpNumber();
            }
        } else {
 
            foreach ($changes as $id) {
                $leave = $wfhRequestService->getWfhById($id);
