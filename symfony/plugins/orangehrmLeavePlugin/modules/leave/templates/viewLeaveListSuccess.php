...
<div class="modal hide" id="conditionalDialog">
  <div class="modal-header">
    <a class="close" data-dismiss="modal">×</a>
    <h3><?php echo __('Leave Request Conditions'); ?></h3>
  </div>
  <div class="modal-body">
    <p>
       ...
       <!-- <textarea name="leaveComment" id="leaveComment" cols="40" rows="4" class="commentTextArea"></textarea>-->
        <div id ="leaveConditions">
             <?php
                    ...
                    ?>
            <label for="txtFromDate"><?php echo __('Date for Reviewval') . $requiredMarker; ?></label>
            <?php echo $leaveConditionForm['txtReviewvalDate']->render() ;?>
         <!--<?php echo $leaveConditionForm ;?>-->
                    <span id="dateError" style="padding-left: 2px;" class="validation-error"></span>
                    <br class="clear" />
                    <label for="txtFromDate"><?php echo __('Conditions') . $requiredMarker; ?></label>
            <?php echo $leaveConditionForm['txtConditions']->render() ;?>
        </div>
  ...
  <div class="modal-footer">
    <?php if ($commentPermissions->canCreate()):?>
    <input type="submit" class="btn" id="conditionSave" value="<?php echo __('Save'); ?>" />
    <?php endif;?>
    <input type="button" class="btn reset" data-dismiss="modal" id="commentCancel" value="<?php echo __('Cancel'); ?>" />
  </div>
</div>
...
<script type="text/javascript">
    ...
    //changes by firoj
        ...
        $('#dialogDeleteBtn').click(function() {
            document.frmList_ohrmListComponent.action = "<?php echo url_for('leave/deleteLeaveHistory');?>";        
            document.frmList_ohrmListComponent.submit();            
        });
        /* Delete confirmation controls: End */
        //end change by firoj
    var postData = {data : <?php echo $sf_data->getRaw('balanceQueryData');?>};
    $.ajax({
       type: "POST",
           ...
           $(document).ready(function() { 
               $('#resultTable tbody tr').each(function(index) {
                   if (index < balanceData.length) {
                       ...
                       var columnNumber;
                       if(postData.data.checkbox==1){
                           columnNumber = 5;
                       }else{
                           columnNumber = 4;
                       }
                       $(this).find('td:nth-child('+columnNumber+')').html(content);
                   }
               });
           });
         }
    ...
    function handleSaveButton() {
        ...
        if (selectedActions > 0) {
          document.frmList_ohrmListComponent.action = "<?php echo url_for('leave/changeLeaveStatus');?>"; 
            document.frmList_ohrmListComponent.submit(); 
        } else {
...
//     $("#leave_txtReviewDate").attr("maxDate","+1D");
    $("#resultTable tbody tr").click(function(){
        var curRow = $(this);
        var td = curRow.find('td a:first-child');
        var isAdmin = '<?php echo $_SESSION['isAdmin'];?>';
        if(isAdmin == 'Yes'){
         maxdate = $(this).find("td:nth-child(2) a").text();
     }
     else{
         maxdate = $(this).find("td:nth-child(1) a").text();
     }
         var dateFinal = maxdate.split(' to');
         var dateFinal1 = dateFinal[0].split(' (');
       $("#leave_txtReviewDate").datepicker('option', { maxDate: dateFinal1[0]})
    });
