...
//         echo "<pre>";
//         print_r($conflictLeaveData);
//        echo $leaveRequest->getEmpNumber();
//        echo "<br />";
//         print_r($leaveRequest->getEmployee()->employeeId);
//         print_r($leaveRequest->getEmployee()->emp_work_email);
//        echo "<pre>";
//        print_r($contactPersons);
//        echo $leaveRequest->getId();
//        echo "<br />";
//        echo $leaveRequest->getNumberOfDays();
//        echo "<br />";
//        echo $leaveRequest->getLeaveDateRange();
//        echo "<br />";
//        print_r($leaveRequest->getLeaveDates());
//        echo "<br />";
//       // print_r($leaveRequest);
    ...
    public function searchLeaveRequests($searchParameters, $page = 1, $isCSVPDFExport = false, $isMyLeaveList = false,
       ...
       // echo"Para :";print_r($searchParameters);exit;
        $this->_markApprovedLeaveAsTaken();
...
//        }
    ...
    public function getPreviousTasks($empNo){
        $q = Doctrine_Query::create()
                ...
                ->from('LeaveTasks t')
                ->where('t.contact_emp_number = ?',$empNo)
                ->andWhere('t.status = ?',0);
        $result = $q->execute();
        return $result;
    }
    public function setTaskStatus($leaveRequest){
        Doctrine_Query::create()
                                ->update('LeaveTasks e')
                                ->set('e.status', 1)
                                ->where('e.leave_request_id = ?', $leaveRequest)
                                ->execute();
    }
    public function getLeaveTasksForDate($emp,$startDate, $endDate){
       /* $q = Doctrine_Query::create()
                        ->select('t.task_name')
                        ->from('LeaveTasks t')
                        ->innerJoin('LeaveRequest lr')
                        ->innerJoin('Leave l')
                        ->where('l.date >= ?',$startDate)
                        ->andWhere('l.date <= ?',$endDate)
                        ->andWhere('t.contact_emp_number = ?',$emp);
        $result = $q->execute();*/
        //$result = array();
        /*$q = Doctrine_Query::create()
               ...
               return $result;
                $sql = Doctrine_Query::create()
                          ->select('t.task_name')
                          ->from('LeaveTasks t')
                          ->where('t.contact_emp_number = ?',$emp)
                          ->andWhereIn('t.leave_request_id = ?',$result);
                return($sql->getSqlQuery());
                $res = $sql->execute();
                return $res;*/
        $q = Doctrine_Query::create()
                     ->select('t.task_name')
                     ->from('LeaveTasks t')
                     ->innerJoin('t.LeaveRequest lr')
                     ->innerJoin('lr.Leave l')
                     ->where('t.leave_request_id = lr.id')
                     ->andWhere('l.leave_request_id = lr.id')
                     ->andWhere('t.contact_emp_number = ?',$emp)
                     ->andWhere('t.status = ?',0)        
    ...
    public function saveLeaveRequestCondition(LeaveRequestConditions $leaveRequestCondition){
        try{
            $leaveRequestCondition->save();
            return $leaveRequestCondition;
    ...
    public function saveLeaveCondition(LeaveConditions $leaveConditions){
        try{
            $leaveConditions->save();
            return $leaveConditions;
        } catch (Exception $e) {
            throw new DaoException($e->getMessage());
        }
    }
