...
class LeaveEntitlementDao extends BaseDao {
    ...
    public function searchLeaveEntitlements(LeaveEntitlementSearchParameterHolder $searchParameters) {
        try {
            ...
            $carrForward =$searchParameters->getCarryForward();
            ...
            if (!empty($fromDate) && !empty($toDate)) {
                ...
                if($carrForward ==1){
                $leavePeriod=$leavePeriodService->getCurrentLeavePeriodByDate(date('Y-m-d',  strtotime($fromDate."-1 day")));
                $params[':fromDate']= $leavePeriod[0];
                $params[':toDate']= $leavePeriod[1];
                $balance=$this->getLeaveBalance($empNumber, $leaveTypeId, $params[':toDate'])->getBalance();
...
>>>>>>> eaeb1055a099d218324d9524e085cad697e82f65
           ...
           if (count($entitlementList) > 0 && $oldentitlementList == null) {
            ...
            if (count($newEmployeeList) > 0 ) {
                ...
                if($oldentitlementList != null){
                    foreach ($newEmployeeList as $empNumber) {
                                $balance=0;
                                if (!isset($allEntitlements[$empNumber])) {
                                    $entitlement = new LeaveEntitlement();
                                    
                                         foreach ($oldentitlementList as $oldentitlement) {
                                             $emp=$oldentitlement['emp_number'];
                                             if($emp==$empNumber){
                                              $balance+=$oldentitlement['no_of_days'] - $oldentitlement['days_used'];
                                            }
                                         }
                                     
                                    $noOfDays = $leaveEntitlement->getNoOfDays();
                                    $noOfDays=$noOfDays+$balance;
                                    $entitlement->setEmpNumber($empNumber);
    ...
    public function bulkAssignLeaveEntitlements($employeeNumbers, LeaveEntitlement $leaveEntitlement,$forward=null) {
        ...
        try {
           ...
           // echo $toDate,"<br />";
            ...
            $leaveEntitlementSearchParameterHolder->setHydrationMode(Doctrine::HYDRATE_ARRAY);
            if($forward !=null ){
             //carry forward set if loop start
              //  echo "<pre>";
             $leaveEntitlementSearchParameterHolder->setCarryForward($forward);
             $oldentitlementList = $this->searchLeaveEntitlements($leaveEntitlementSearchParameterHolder);
             //print_r($oldentitlementList);
             $leaveEntitlementSearchParameterHolder->setCarryForward(0);
             $entitlementList=$this->searchLeaveEntitlements($leaveEntitlementSearchParameterHolder);
            // echo "<br />new list<br />";
            // print_r($entitlementList);
            // exit();
             ...
             //carry forward set if loop end
             }else{
             //carry not set   else loop start
              $entitlementList = $this->searchLeaveEntitlements($leaveEntitlementSearchParameterHolder); 
              if (count($entitlementList) > 0) {
                foreach ($entitlementList as $updateEntitlement) {
...
*/
    ...
    public function getLeaveBalance($empNumber, $leaveTypeId, $asAtDate, $date = NULL) {
        ...
        $sql = 'SELECT le.no_of_days AS entitled, ' .
                ...
                'sum(IF(l.status = 6, lle.length_days, 0)) AS conditional_approved, ' .
        ...
        if (is_array($dateLimits) && count($dateLimits) > 0) {
            $sql .= ' UNION ALL ' .
                    ...
                    'sum(IF(l.status = 6, l.length_days, 0)) AS conditional_approved, ' .
        ...
        $sql = 'SELECT sum(a.entitled) as entitled, sum(a.used) as used, sum(a.scheduled) as scheduled, sum(a.conditional_approved) as conditional_approved,' . 
        ...
        if ($result) {
            if ($statement->rowCount() > 0) {
                ...
                if (!empty($result['conditional_approved'])) {
                    $balance->setConditional_approved($result['conditional_approved']);
                }
    ...
    public function getEndDate($empNumber,$leaveTypeId){
        $conn = Doctrine_Manager::connection()->getDbh();
        $sql = "SELECT le.to_date FROM ohrm_leave_entitlement le WHERE le.emp_number = ? AND le.leave_type_id = ? AND le.deleted = 0";
        $stmt = $conn->prepare($sql);
        $result = $stmt->execute(array($empNumber,$leaveTypeId));
//        $res = $stmt->fetchAll();
//        var_dump($res[0]["to_date"]);exit;
