 ...
 <script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        var intialTo    = $('#date_to').val();
            ...
            var date= intialFrom;
                        var tmp = date.split("-").reverse().join("-");
            var d=new Date(tmp);
                
            var dd=d.getDate();
            var mm=d.getMonth()+1;
            var yy=d.getFullYear();
            var newdate = mm<10?(dd<10?yy+"-0"+mm+"-0"+dd:yy+"-0"+mm+"-"+dd):(dd<10?yy+"-"+mm+"-0"+dd:yy+"-"+mm+"-"+dd);            
//            var newdate= yy+"-0"+mm+"-0"+dd;
//                var newdate =    yy+"-"+mm+"-"+dd;   
            var date1 = new Date(newdate);
            var date2 = new Date(intialTo);

        if(date1 >= new Date(yy +'-04-01') && date1 <= new Date(yy+'-06-30')){

               $('#period').val('01-04-'+yy+'$$'+'30-06-'+yy);         
                        }
                        else if(date1 >=  new Date(yy+'-07-01') && date1 <= new Date(yy+'-09-30')){
   $('#period').val('01-07-'+yy+'$$'+'30-09-'+yy);         
                        }
                        else if(date1 >= new Date(yy+'-10-01') && date1 <= new Date(yy+'-12-31')){
               $('#period').val('01-10-'+yy+'$$'+'31-12-'+yy);         
                        }
                        else{
               $('#period').val('01-01-'+(yy)+'$$'+'31-03-'+(yy));         
                        }
