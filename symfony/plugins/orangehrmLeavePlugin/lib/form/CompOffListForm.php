...
/**
 * Form class for comp-off list
 */
class CompOffListForm extends sfForm {

    const MODE_MY_LEAVE_LIST = 'my_leave_list';
    const MODE_ADMIN_LIST = 'default_list';
    //const MODE_ADMIN_LIST = 0;
    private $mode;
    private $list = null;
    ...
    public function configure() {
        ...
        //From and to dates are removed : changes by Rahul
        // From and To Date
   /*     $widgets['calFromDate'] = new ohrmWidgetDatePicker(array(), array('id' => 'calFromDate'));
       ...
       // }
        ...
        if ($this->mode != self::MODE_MY_LEAVE_LIST) {
            ...
            $widgets['eeo_category'] = new sfWidgetFormSelect(array('choices' => $eeoCategories));
            $validators['eeo_category'] = new sfValidatorChoice(array('required' => false, 'choices' => array_keys($eeoCategories)));
            $labels['eeo_category'] = __('Department');
        ...
        $this->getWidgetSchema()->setNameFormat('compOffList[%s]');
    ...
    public function getTitle() {
        ...
        if ($this->mode === self::MODE_MY_LEAVE_LIST) {
            $title = __('My Comp-off List');
        } else {
            $title = __('Comp-off List');
        }
    ...
    /**
     * Returns the set of action buttons associated with each mode of the comp-off list(reference of leave list)
     *
    ...
    /*
     * To store the checkbox values in an array
     * code by rahul
     */
