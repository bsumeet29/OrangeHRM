...
//                $i++;
//            }
//            $startDate = new DateTime(date('Y')."-4-1");
//            $endDate=new DateTime(date('Y',  strtotime("+1 year"))."-3-31");
//            $tempDate = $startDate;
//            $i= 0;
//            while( $tempDate <  $endDate){
//                
//                 if($i==0){
//                     $projectedSatrtDate =$tempDate;
//                     
//                 }else{
//                  $projectedSatrtDate = new DateTime(date('Y-m-d',  strtotime($tempDate->format('Y-m-d')."+1 day")));
//                          
//                 }
//                 $projectedEndDate = new DateTime(date('Y-m-d',  strtotime($projectedSatrtDate->format('Y-m-d')." +3 months -1 day")));
//                 $tempDate = $projectedEndDate;
//                 $leavePeriodList[] = array($projectedSatrtDate->format('Y-m-d') , $projectedEndDate->format('Y-m-d'));
//                 $i++;
//            }
           ...
           // echo "<pre>";
           // print_r($leavePeriodList);
           
            
            
            if(date('m')>=1 && date('m')<=3){
            $leavePeriodList[]=array(date('Y',strtotime('-1 year'))."-04-01",date('Y',strtotime('-1 year'))."-06-30");
            $leavePeriodList[]=array(date('Y',strtotime('-1 year'))."-07-01",date('Y',strtotime('-1 year'))."-09-30");
            $leavePeriodList[]=array(date('Y',strtotime('-1 year'))."-10-01",date('Y',strtotime('-1 year'))."-12-31");
            $leavePeriodList[]=array(date('Y')."-01-01",date('Y')."-03-31");
            }else{
            $leavePeriodList[]=array(date('Y')."-04-01",date('Y')."-06-30");
            $leavePeriodList[]=array(date('Y')."-07-01",date('Y')."-09-30");
            $leavePeriodList[]=array(date('Y')."-10-01",date('Y')."-12-31");    
            $leavePeriodList[]=array(date('Y',  strtotime('+1 year'))."-01-01",date('Y',  strtotime("+1 year"))."-03-31");
           }
