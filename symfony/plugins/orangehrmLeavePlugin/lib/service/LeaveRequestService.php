...
class LeaveRequestService extends BaseService {
    ...
    public function getCompoffRequestDao(){
        if(!($this->compoffRequestDao instanceof CompoffRequestDao)){
    ...
    public function getLeaveTaskService(){
        if(is_null($this->leaveTaskService)){
            $this->leaveTaskService = new LeaveTasksService();
        }
        return $this->leaveTaskService;
    }
    public function setLeaveTaskService(LeaveTasksService $leaveTaskService){
    ...
    public function saveLeaveRequestCondition(LeaveRequest $leaveRequest, $condition, $reviewDate){
       // $leaveRequesttObject = $this->fetchLeaveRequest($leaveRequestId);
         $leaveRequestCondition = new LeaveRequestConditions();
           // $leaveRequestCondition->setId(" ");
            $leaveRequestCondition->setReviewvalDate($reviewDate);
            $leaveRequestCondition->setConditions($condition);
            $leaveRequestCondition->setLeaveRequest($leaveRequest);
            $leaveRequestCondition->setDate(date('Y-m-d H:i:s'));
        return $this->getLeaveRequestDao()->saveLeaveRequestCondition($leaveRequestCondition);
    }
    public function saveLeaveCondition(Leave $leave,$leaveId,$condition,$reviewDate){
        $leaveCondition = new LeaveConditions();
        $leaveCondition->setReviewvalDate($reviewDate);
        $leaveCondition->setConditions($condition);
        $leaveCondition->setLeave($leave);
        $leaveCondition->setDate(date('Y-m-d H:i:s'));
        return $this->getLeaveRequestDao()->saveLeaveCondition($leaveCondition);
    ...
    public function getPreviousTasks($empNo){
        $tasks = $this->getLeaveRequestDao()->getPreviousTasks($empNo);
        return $tasks;
    }
    public function getLeaveTasksForDate($empNumber,$startDate,$endDate){
        $tasks = $this->getLeaveRequestDao()->getLeaveTasksForDate( $empNumber,$startDate, $endDate);
        return $tasks;
    }
    public function setTaskStatus($leaveRequest){
        $this->getLeaveRequestDao()->setTaskStatus($leaveRequest);
    }
    /**
     *
    ...
    public function getEmployeeAllowedToApplyLeaveTypes(Employee $employee) {
        try {
           ...
           $leaveTypesCopy = $leaveTypes     = $leaveTypeService->getLeaveTypeList();
                ...
                if($balance->getEntitled() > 0 ) { 
                     if($leaveType->getId() == '4' ) {
                        if( $balance->getBalance() > 0 ) {
                            foreach($leaveTypesCopy as $leaveTypeCopy){
                                $balancecopy= $leaveEntitlementService->getLeaveBalance($employee->getEmpNumber(), $leaveTypeCopy->getId());
                                if($leaveTypeCopy->getId() == '1'){
                                    if($balancecopy->getBalance() > 0){
                                        unset($leaveTypeList);
                            $leaveTypeList[0] = $leaveType;
                            $leaveTypeList[1] = $leaveTypeCopy;
                            $leaveTypeList[2] = $nplLeave;
    ...
    public function searchLeaveRequests($searchParameters, $page = 1, $isCSVPDFExport = false, $isMyLeaveList = false, 
        ...
        //echo"par : ";print_r($searchParameters);exit;
        $this->getCompoffRequestDao()->markExpiredCompoff();
    ...
    public function deleteLeaveHistory($ids){
       $this->getLeaveRequestDao()->deleteLeaveHistory($ids);
    }
    /**
     *
     * @param array $changes
    ...
    private function _notifyLeaveStatusChange($eventType, $workflow, $leaveList, $performerType, $performerId, $requestType) {
        ...
        $leaveTasks= $request->getLeaveTasks();
        $employeeIds=array();
        foreach ($leaveTasks as $task){
            $id=$task->getContactEmpNumber();
            if(!in_array($id, $employeeIds)){
                $employeeIds[]=$id;
            }
        }
      // echo"id:";print_r($employeeIds);exit;
