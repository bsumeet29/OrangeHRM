        ...
        foreach ($this->statusCounter as $status => $count) {
            if (!is_null($status)) {
              ...
              /*  if($status == '1'){
                    ...
                    $supervisorName = ' Approval Pending with : '.$supervisor;
                } */
                $statusStrings[] = __(ucwords(strtolower(Leave::getTextForLeaveStatus($status)))) . "(" . number_format($count, 2) .")". $supervisorName;
            }
        }
        
        return implode(', ', $statusStrings);
    }
    public function getApprovalPendingWith(){
          $this->_fetchLeave();
        ...
        foreach ($this->statusCounter as $status => $count) {
            if (!is_null($status)) {
               // echo"status :".$status;exit;
                $supervisor = null;
                if($status == '1'){
                    $supervisorName = $this->getSupervisorName();
                   // $supervisorName = ' Approval Pending with : '.$supervisor;
                }
                $statusStrings[] =  $supervisorName;
            }
        }
        return implode(', ', $statusStrings);
    }
